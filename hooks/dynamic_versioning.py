    ...
    **kwargs,  # pylint: disable=unused-argument
    ...
    with stash_unstaged_changes(files):
        try:
            ...
            output = subprocess.check_output(arguments, stderr=STDOUT).decode("utf-8")
            print("Finished running command")
            out(output)
            version = re.findall(r"[0-9]+.[0-9]+.[0-9]+", output)[0]
            subprocess.check_output(
                f'git tag -d v{version} || echo "Tag not found"', shell=True
            ).decode("utf-8")
            out(
                subprocess.check_output(f'git tag -m "" v{version}', shell=True).decode(
                    "utf-8"
                )
            )
